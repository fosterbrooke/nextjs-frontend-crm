  ...
  return (
    ...
    >
      ...
      <div className="flex p-4 m-2 font-arial justify-center border border-borderGreyColor">
        ...
        <div className="inline-flex flex-col justify-center gap-x-2 w-4/5">
          ...
          <div className="flex items-center">
            {workItems.map((item, index) => (
              <span
                key={index}
                className="bg-red-200 rounded-3xl font-bold text-zinc-800 text-xs m-1 p-1"
              >
                {item}
              </span>
            ))}
        ...
        <span
          className="flex items-center cursor-pointer px-3 py-1 my-8 bg-primary rounded-full"
          onClick={() => setModalOpen(true)}
        >
          <p className="text-white">Schedle meeting</p>
        </span>
      </div>
